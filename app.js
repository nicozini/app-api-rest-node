...
// Solicitudes por PUT
// Modularizo Métodos
// Solicitudes por DELET
// Incorporación de Middlewares APP (express.json() y urlencoded)
...
// Middlewares propios Express
// Middleware para parsear la información que llega desde el body (post) a JSON
app.use(express.json());
// Middleware para trabajar con los datos de la URL
app.use(express.urlencoded({extended: true}));
// Middleware para configurar recursos estáticos
app.use(express.static('public'));


// Middlewares de terceros en Express
app.use(morgan('tiny'));
console.log('Morgan habilitado...');


// Middleware Nivel APP propio para explicar middlewares
app.use(logger);
...
app.delete('/api/usuarios/:id', (req,res) => {
   // 1 - Verifico si existe el id con el dato a elminar
   let usuario = existeUsuario(req.params.id);
   if (!usuario) {
       res.status(404).send('El usuario no fue encontrado');
       return;
    ...
    // 2 - Identificar el indice del id para eliminarlo
...
// MÉTODOS QUE VAN EN OTRO MODULO
